...
//     const id = props.custom ?
// url = !props.custom ? defOpt.id : defOpr.host + defOpt.pathname + S;
// ({props}) => {
// ({props})=>(
//     let tUrl = custom.url instanceof URL ? .}const.url :
//     defOpt.URL;
//         if(tUrl instanceof URL) {
//             return tUrl.host +  parent['custom'].ptUrl.pathname
//         } else {
//             return defOpt.stringUrl(defOpt.id || null, defOpt.endpoint);
//         }
//         return url.host, + className  < ended()
// };
// return defOpt.stringUrl(props.cusotm.id, defOpt.url.);
...
/*
 ...
 let url:string;
 let fallback = {};
 const ret = await data.map(async (item) => {
 url = API + '/' + item.id;
 const myRet = fetcher(url);
 fallback[url] = (await myRet);
 return {[url]: fallback[url]}
 });
 console.log(await ret);
 console.log(await data);

 console.log(await (fallback), await ret);
 if (await ((Object.keys(fallback)).length == (Object.keys(data)).length)) {
 return (fallback);
 }

 };
 let ret = adata().then(async (r)=>r);
 let testRet;
 do {
 await ret.delay(50);
 console.log(testRet);
 }while (testRet !== undefined);

 console.log((await ret), {[API]: (await data)});
 return {
 props: {
 fallback: await Object.assign((ret), {[API]: data})
 }
 };
