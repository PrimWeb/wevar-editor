...
const Indicators = styled.div<{ bound?: 'row' | 'column' }>`
  ...
  span {
    ...
    &:nth-child(1) {
      ${(props) => props.bound ? props.bound === 'row' ? `
                left: 50%;
                top: -5px;
...
export const Resizer = ({ propKey, children, ...props }: any) => {
    ...
    return (<Resizable
        ...
        onResizeStart={(e) => {
            ...
            editingDimensions.current = {
                width: dom.getBoundingClientRect().width, height: dom.getBoundingClientRect().height,
            };
    ...
    >
        {children}
        {active && (<Indicators bound={fillSpace === 'yes' ? parentDirection : false}>
            <span></span>
            <span></span>
            <span></span>
            <span></span>
